 ...
 * @ORM\Entity(repositoryClass="Coniq\Tools\CoffeeBreak\Repository\CoffeeBreakPreferenceRepository")
 */
class CoffeeBreakPreference
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity="Coniq\Tools\CoffeeBreak\Entity\StaffMember", inversedBy="preferences")
    ...
    public function __construct($type, $subType, StaffMember $requestedBy, array $details = [])
    {
        ...
        $types = $preferenceType->getTypes();
        
        $preferenceDrinkTypes = new PreferenceDrinkTypes();
        $drink_types = $preferenceDrinkTypes->getDrinkTypes();

        $preferenceFoodTypes = new PreferenceFoodTypes();
        $food_types = $preferenceFoodTypes->getFoodTypes();
        ...
        if ($type == "food") {
            if (!in_array($subType, $food_types)) {
                throw new \InvalidArgumentException;
            }
        } else {
            if (!in_array($subType, $drink_types)) {
                throw new \InvalidArgumentException;
            }
        }
